 ...
 *
 * By default, shows an application About dialog,
...
static void redraw(void) {
	...
	draw_string(offset, version_str, MARKUP_TEXT_STATE_BOLD, RAZION_TEXT_PRIMARY);
	...
	for (char ** str = body_text; *str; ++str) {
		if (**str == '-') {
			...
			draw_string(offset, *str+1, 0, RAZION_ACCENT_HOVER);
			offset += 20;
		} else {
			draw_string(offset, *str, 0, RAZION_TEXT_SECONDARY);
			offset += 20;
		}
	}
