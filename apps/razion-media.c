...
static char path[512];static size_t path_length;static int running=1,editing=1,hover=-1,pressed=-1,focus_id=10;static pid_t player;
...
static void move_focus(int direction){static const int order[]={10,0,1,2};int at=0;for(int i=0;i<4;i++)if(order[i]==focus_id)at=i;focus_id=order[(at+direction+4)%4];editing=focus_id==10;}
static void redraw(void){struct decor_bounds b;decor_get_bounds(window,&b);draw_fill(ctx,RAZION_BACKGROUND);int x=b.left_width+32,top=b.top_height;
	razion_draw_accent_bar(ctx,x,top+22,90);tt_set_size(bold,24);tt_draw_string(ctx,bold,x,top+44,"Media Player",RAZION_TEXT_PRIMARY);tt_set_size(font,11);tt_draw_string(ctx,font,x,top+66,"Native low-overhead PCM playback",RAZION_TEXT_SECONDARY);
	if(focus_id==10&&window->focused){
		razion_draw_focus(ctx,x,top+96,window->width-b.width-64,52,9);
	}
	razion_draw_card(ctx,x,top+96,window->width-b.width-64,52,editing?RAZION_SURFACE_HOVER:RAZION_SURFACE);tt_set_size(font,13);
	...
	const char*names[]={"Play","Stop","Open Music folder"};for(int i=0;i<3;i++){int bx=x+i*148;unsigned state=RAZION_CONTROL_NORMAL;if(i==pressed)state|=RAZION_CONTROL_PRESSED;if(i==hover)state|=RAZION_CONTROL_HOVER;if(i==focus_id&&window->focused)state|=RAZION_CONTROL_FOCUSED;if(i==0)state|=RAZION_CONTROL_PRIMARY;if((i==0&&!path[0])||(i==1&&player<=0))state|=RAZION_CONTROL_DISABLED;razion_draw_button(ctx,font,bx,top+174,136,42,names[i],state);}
	tt_set_size(font,11);razion_draw_status_dot(ctx,x,top+247,player>0?RAZION_SUCCESS:RAZION_TEXT_SECONDARY);tt_draw_string(ctx,font,x+16,top+254,status,RAZION_TEXT_SECONDARY);tt_draw_string(ctx,font,x,top+282,"Supported: raw 48 kHz signed 16-bit stereo PCM. Tab selects controls.",RAZION_TEXT_SECONDARY);
...
int main(void){yctx=yutani_init();if(!yctx)return 1;init_decorations();struct decor_bounds b;decor_get_bounds(NULL,&b);window=yutani_window_create(yctx,560+b.width,350+b.height);window->decorator_flags|=DECOR_FLAG_NO_MAXIMIZE;
	...
	while(running){if(player>0&&waitpid(player,NULL,WNOHANG)==player){player=0;snprintf(status,sizeof(status),"Playback finished.");redraw();}yutani_msg_t*m=yutani_poll(yctx);if(!m)continue;switch(m->type){case YUTANI_MSG_KEY_EVENT:{struct yutani_msg_key_event*k=(void*)m->data;if(k->wid!=window->wid||k->event.action!=KEY_ACTION_DOWN)break;int shift=k->event.modifiers&(KEY_MOD_LEFT_SHIFT|KEY_MOD_RIGHT_SHIFT);if(k->event.keycode==KEY_ESCAPE)running=0;else if(k->event.keycode=='\t'){move_focus(shift?-1:1);redraw();}else if(k->event.keycode==KEY_ARROW_LEFT||k->event.keycode==KEY_ARROW_UP){move_focus(-1);redraw();}else if(k->event.keycode==KEY_ARROW_RIGHT||k->event.keycode==KEY_ARROW_DOWN){move_focus(1);redraw();}else if(k->event.key=='\n'){activate(focus_id);redraw();}else if(editing&&(k->event.key=='\b'||k->event.keycode==KEY_BACKSPACE)&&path_length){path[--path_length]='\0';redraw();}else if(editing&&k->event.key>=0x20&&k->event.key<0x7f&&path_length+1<sizeof(path)){path[path_length++]=k->event.key;path[path_length]='\0';redraw();}break;}
	case YUTANI_MSG_WINDOW_MOUSE_EVENT:{struct yutani_msg_window_mouse_event*e=(void*)m->data;if(e->wid!=window->wid)break;if(decor_handle_event(yctx,m)==DECOR_CLOSE)running=0;int over=hit(e->new_x,e->new_y);if(e->command==YUTANI_MOUSE_EVENT_DOWN){pressed=over;if(over>=0){focus_id=over;editing=over==10;}}else if(e->command==YUTANI_MOUSE_EVENT_LEAVE){hover=-1;pressed=-1;}else if(e->command==YUTANI_MOUSE_EVENT_CLICK||e->command==YUTANI_MOUSE_EVENT_RAISE){if(over==pressed)activate(over);pressed=-1;}hover=over;redraw();break;}
