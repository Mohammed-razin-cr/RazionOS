...
static void draw_string(int y, const char * string, int mode, uint32_t color, int size) {
	struct decor_bounds bounds;
	decor_get_bounds(window, &bounds);

	struct MarkupState * renderer = markup_setup_renderer(NULL, 0, 0, 0, 1);
	markup_set_base_font_size(renderer, size);
	markup_set_base_state(renderer, mode);
	markup_push_string(renderer, string);
	int calcwidth = markup_finish_renderer(renderer);

	renderer = markup_setup_renderer(ctx, bounds.left_width + center(calcwidth, width), bounds.top_height + 30 + y + size, color, 0);
	markup_set_base_font_size(renderer, size);
	markup_set_base_state(renderer, mode);
	markup_push_string(renderer, string);
...
static void redraw(void) {
	...
	for (char ** copy_str = body_text; *copy_str; ++copy_str) {
		if (**copy_str == '-') {
			...
			draw_string(offset, *copy_str+1, 0, rgb(0,0,255), 13);
			offset += 20;
		} else if (**copy_str == '#') {
			draw_string(offset, *copy_str+1, MARKUP_TEXT_STATE_BOLD, rgb(0,0,0), 20);
			offset += 20;
		} else {
			draw_string(offset, *copy_str, 0, rgb(0,0,0), 13);
			offset += 20;
		}
	}
...
static void load_page(int page) {
	...
	switch (page) {
		case 0:
			_prev_enabled = 0;
			title_str = "Welcome to RazionOS!";
			icon = &logo;
			body_text[i++] = "#Welcome to RazionOS 0.1 Alpha";
			body_text[i++] = "";
			...
			body_text[i++] = "";
			body_text[i++] = "RazionOS is an open-source desktop operating system based on ToaruOS.";
			body_text[i++] = "Upstream components use the NCSA/University of Illinois license.";
			body_text[i++] = "";
		...
		case 1:
			icon = &logo;
			body_text[i++] = "RazionOS is in alpha development and builds on the ToaruOS project.";
			body_text[i++] = "The underlying system was written by K. Lange and ToaruOS contributors";
			body_text[i++] = "over many years. Some features";
		...
		case 4:
			icon = &terminal;
			circle(70, 170, 60);
			body_text[i++] = "RazionOS provides a Unix-like environment. You can find familiar";
			body_text[i++] = "command-line tools by opening a terminal. The rzsh shell identity";
			body_text[i++] = "(powered by esh) provides history, syntax highlighting, and tab";
		...
		case 5:
			icon = &package;
			circle(70, 250, 60);
			body_text[i++] = "Many third-party packages ported for ToaruOS remain compatible";
			body_text[i++] = "and are available from the upstream repositories. You can use the";
			body_text[i++] = "Package Manager to install GCC, Doom, Quake, and more.";
			body_text[i++] = NULL;
			break;
		case 6:
			icon = &mouse_drag;
			body_text[i++] = "With Razion Desktop, you can drag most windows by";
			...
			body_text[i++] = "";
			body_text[i++] = "Note that if you are running RazionOS in a virtual machine, your";
			body_text[i++] = "host operating system configuration may conflict with modifier";
			body_text[i++] = "keys in RazionOS.";
			body_text[i++] = NULL;
			break;
		case 7:
			icon = NULL;
			...
			body_text[i++] = "";
			body_text[i++] = "Press \"Exit\" to close this window and explore RazionOS.";
			body_text[i++] = NULL;
			break;
		default:
			exit(0);
