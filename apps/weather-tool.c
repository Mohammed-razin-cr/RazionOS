...
int main(int argc, char * argv[]) {
	...
	Value * cityValue = JSON_KEY(config, "city");
	Value * keyValue = JSON_KEY(config, "key");
	Value * unitsValue = JSON_KEY(config, "units");
	if (!cityValue || cityValue->type != JSON_TYPE_STRING ||
	    !keyValue || keyValue->type != JSON_TYPE_STRING ||
	    !unitsValue || unitsValue->type != JSON_TYPE_STRING) {
		fprintf(stderr, "Invalid weather config data\n");
		return 1;
	}

	char * city = cityValue->string;
	char * key = keyValue->string;
	char * units = unitsValue->string;
	if (!key || !*key) {
		fprintf(stderr, "No OpenWeatherMap API key configured; run weather-configurator first\n");
		return 1;
	}
	char cmdline[1024];
