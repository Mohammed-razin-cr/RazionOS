...
static int year,month,today_year,today_month,today_day,running=1,hover;
...
static void redraw(void){struct decor_bounds b;decor_get_bounds(window,&b);draw_fill(ctx,RAZION_BACKGROUND);int left=b.left_width+30,top=b.top_height;
	char title[64];snprintf(title,sizeof(title),"%s %d",months[month],year);tt_set_size(bold,24);tt_draw_string(ctx,bold,left,top+44,title,RAZION_TEXT_PRIMARY);
	draw_rounded_rectangle(ctx,window->width-b.right_width-98,top+18,32,32,7,hover==1?RAZION_SURFACE_HOVER:RAZION_SURFACE_SECONDARY);
	draw_rounded_rectangle(ctx,window->width-b.right_width-58,top+18,32,32,7,hover==2?RAZION_SURFACE_HOVER:RAZION_SURFACE_SECONDARY);
	tt_set_size(bold,18);tt_draw_string(ctx,bold,window->width-b.right_width-87,top+41,"‹",RAZION_TEXT_PRIMARY);tt_draw_string(ctx,bold,window->width-b.right_width-47,top+41,"›",RAZION_TEXT_PRIMARY);
	...
	tt_set_size(font,10);tt_draw_string(ctx,font,left,window->height-b.bottom_height-20,"Calendar uses the current system date and time.",RAZION_TEXT_SECONDARY);
...
int main(void){time_t now=time(NULL);struct tm*local=localtime(&now);year=today_year=local?local->tm_year+1900:2026;month=today_month=local?local->tm_mon:0;today_day=local?local->tm_mday:1;
	...
	while(running){yutani_msg_t*m=yutani_poll(yctx);if(!m)continue;switch(m->type){case YUTANI_MSG_KEY_EVENT:{struct yutani_msg_key_event*k=(void*)m->data;if(k->wid==window->wid&&k->event.action==KEY_ACTION_DOWN){if(k->event.keycode==KEY_ESCAPE)running=0;else if(k->event.keycode==KEY_ARROW_LEFT){change_month(-1);redraw();}else if(k->event.keycode==KEY_ARROW_RIGHT){change_month(1);redraw();}}break;}
	case YUTANI_MSG_WINDOW_MOUSE_EVENT:{struct yutani_msg_window_mouse_event*e=(void*)m->data;if(e->wid!=window->wid)break;if(decor_handle_event(yctx,m)==DECOR_CLOSE)running=0;hover=hit(e->new_x,e->new_y);if(e->command==YUTANI_MOUSE_EVENT_CLICK&&hover){change_month(hover==1?-1:1);}redraw();break;}
