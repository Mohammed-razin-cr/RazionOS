		...
		if (show_hidden || (ent->d_name[0] != '.')) {
			...
			if (S_ISDIR(statbuf.st_mode)) {
				...
				if (is_desktop_background && has_extension(f, ".launcher")) {
					...
					} else if (has_extension(f, ".avi")) {
						sprintf(f->icon, "video");
						sprintf(f->launcher, "exec razion-recording-player");
						sprintf(f->filetype, "AVI Video Recording");
...
static void _menu_action_about(struct MenuEntry * entry) {
	...
	if (!fork()) {
		...
		char *args[] = {
			...
			"--logo", "folder",
			"--icon", "star",
			"--name", "RazionOS " APPLICATION_TITLE,
			"--at", coords,
			"--",
			"© 2018-2026 K. Lange",
			"-",
