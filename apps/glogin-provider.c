...
static char * WALLPAPER = "/usr/share/wallpapers/razion-premium.jpg";
...
void draw_text_box(gfx_context_t * ctx, struct text_box * tb) {
	...
	if (tb->is_focused) {
		draw_rounded_rectangle(ctx, 1 + x, 1 + y, tb->width - 2, tb->height - 2, 4, RAZION_FOCUS);
		draw_rounded_rectangle(ctx, 2 + x, 2 + y, tb->width - 4, tb->height - 4, 4, RAZION_SURFACE);
	} else {
		draw_rounded_rectangle(ctx, 1 + x, 1 + y, tb->width - 2, tb->height - 2, 4, RAZION_BORDER);
		draw_rounded_rectangle(ctx, 2 + x, 2 + y, tb->width - 4, tb->height - 4, 4, RAZION_SURFACE_SECONDARY);
	}
	/* Line width 2? */
	char * text = tb->buffer;
...
void draw_login_container(gfx_context_t * ctx, struct login_container * lc) {
	...
	if (lc->show_error) {
		...
		tt_set_size(tt_font_thin, 13);
		tt_draw_string(ctx, tt_font_thin, lc->x + (lc->width - tt_string_width(tt_font_thin, error_message)) / 2, lc->y + 6 + EXTRA_TEXT_OFFSET - 1, error_message, RAZION_ERROR);
	}
...
redo_everything:
	...
	while (1) {
...

		{
			struct utsname u;
			uname(&u);
			char * os_name_ = "RazionOS 0.1 Alpha";
			snprintf(kernel_v, 512, "%s // %.120s %.120s", os_name_, u.sysname, u.release);
		}
		...
		struct text_box username_box = { (BOX_WIDTH - 170) / 2, 30, 170, 20, RAZION_TEXT_PRIMARY, NULL, 0, 0, 0, username, "Username" };
		struct text_box password_box = { (BOX_WIDTH - 170) / 2, 58, 170, 20, RAZION_TEXT_PRIMARY, NULL, 0, 1, 0, password, "Password" };
