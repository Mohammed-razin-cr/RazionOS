/**
 * @brief Isolated Ollama provider adapter for the Razion AI Engine.
 *
...
#define OLLAMA_CONFIG "/etc/razion-ai-ollama.conf"
#define OLLAMA_ENDPOINT "razion-ai-provider-ollama"
#define OLLAMA_MAX_HTTP_RESPONSE 65536
#define OLLAMA_MAX_JSON_REQUEST 8192
...
static ollama_context_t ollama = {
	.host = "127.0.0.1",
	.port = 11434,
	.timeout_ms = 4000,
	.max_tokens = 192,
...
static int ollama_initialize(void * opaque, const char * config_path) {
	ollama_context_t * context = opaque;
	confreader_t * config = confreader_load(config_path);
	if (config) {
		strncpy(context->host,
			confreader_getd(config, "ollama", "host", "127.0.0.1"),
			sizeof(context->host) - 1);
		context->port = confreader_intd(config, "ollama", "port", 11434);
		strncpy(context->model,
			confreader_getd(config, "ollama", "model", ""),
			sizeof(context->model) - 1);
		context->timeout_ms =
			confreader_intd(config, "ollama", "timeout_ms", 4000);
		context->max_tokens =
			confreader_intd(config, "ollama", "max_tokens", 192);
		context->allow_remote =
			confreader_intd(config, "ollama", "allow_remote", 0);
...
static void ollama_shutdown(void * opaque) {
	(void)opaque;
}

static int http_request(
	ollama_context_t * context,
	const char * method,
	const char * path,
	const char * body,
	char ** response_body,
	...
	*status_code = 0;
	...
	if (!stream) {
		close(socket_fd);
		return http_error("fdopen");
	}

	size_t body_length = body ? strlen(body) : 0;
	int write_failed = fprintf(stream,
		"%s %s HTTP/1.0\r\n"
		"Host: %s:%d\r\n"
		"User-Agent: RazionOS-Ollama-Adapter/0.1\r\n"
		"Accept: application/json\r\n"
		"Content-Type: application/json\r\n"
		"Content-Length: %u\r\n"
		"Connection: close\r\n\r\n",
		method, path, context->host, context->port, (unsigned)body_length) < 0;
	if (!write_failed && body_length &&
		fwrite(body, 1, body_length, stream) != body_length) {
		write_failed = 1;
	}
	fflush(stream);
	if (write_failed) {
	...
	if (!fgets(line, sizeof(line), stream) ||
		sscanf(line, "HTTP/%*s %d", status_code) != 1) {
		fclose(stream);
	...
	if (!have_content_length || content_length > OLLAMA_MAX_HTTP_RESPONSE) {
		fclose(stream);
	...
	*response_body = malloc(content_length + 1);
	...
	while (used < content_length) {
		size_t received =
			fread(*response_body + used, 1, content_length - used, stream);
		if (!received) {
			free(*response_body);
			*response_body = NULL;
			fclose(stream);
...
static int discover_model(
	...
	if (status != 200) {
		free(response);
		snprintf(detail, detail_size,
			"Ollama health returned HTTP %d", status);
		return -1;
	}
	...
	free(response);
	struct JSON_Value * models = object_value(root, "models", JSON_TYPE_ARRAY);
	if (!models || !models->array->length) {
		if (root) json_free(root);
		snprintf(detail, detail_size, "Ollama is running but no local model is installed");
		return -1;
	}
	...
	foreach (node, models->array) {
		...
		struct JSON_Value * model = object_value(entry, "model", JSON_TYPE_STRING);
	...
	if (!first || !configured_found) {
		json_free(root);
		snprintf(detail, detail_size,
			context->model[0] ? "Configured Ollama model is not installed" :
			"Ollama did not return a usable model");
		return -1;
	}
...
static int ollama_health(void * opaque, char * detail, size_t detail_size) {
	ollama_context_t * context = opaque;
	char model[sizeof(context->model)];
	return discover_model(context, model, sizeof(model), detail, detail_size);
}

static int ollama_available(void * opaque) {
	char detail[RAZION_AI_PROVIDER_MAX_DETAIL];
	return ollama_health(opaque, detail, sizeof(detail)) == 0;
}

static uint32_t ollama_latency(void * opaque) {
	return ((ollama_context_t *)opaque)->last_latency_ms;
}
...
static const char * operation_instruction(razion_ai_operation_t operation) {
	switch (operation) {
		...
		case RAZION_AI_OP_CLASSIFY_INTENT:
			return "Classify the command into exactly one token: open-files, open-terminal, open-settings, open-downloads, check-memory, create-python-project, or unknown. Command:";
...
static int ollama_generate(
	...
	char escaped_prompt[OLLAMA_MAX_JSON_REQUEST / 2];
	...
	char * response = NULL;
	int status = 0;
	if (http_request(context, "POST", "/api/generate", body,
		&response, &status)) {
		result->status = RAZION_AI_STATUS_PROVIDER_UNAVAILABLE;
		strncpy(result->output, "Ollama request failed or timed out.",
			sizeof(result->output) - 1);
		return result->status;
	}
	gettimeofday(&end, NULL);
	...
	if (status != 200 || !root) {
		if (root) json_free(root);
		result->status = RAZION_AI_STATUS_PROVIDER_UNAVAILABLE;
		strncpy(result->output, "Ollama returned an unsuccessful response.",
			sizeof(result->output) - 1);
		return result->status;
	}

	struct JSON_Value * output = object_value(root, "response", JSON_TYPE_STRING);
	if (!output || !output->string[0]) {
		struct JSON_Value * error = object_value(root, "error", JSON_TYPE_STRING);
		result->status = RAZION_AI_STATUS_PROVIDER_UNAVAILABLE;
		strncpy(result->output,
			error ? error->string : "Ollama returned no generated text.",
			sizeof(result->output) - 1);
		json_free(root);
		return result->status;
	}
...
static int ollama_chat(
	void * opaque,
	const razion_ai_provider_request_t * request,
	razion_ai_provider_result_t * result) {
	return ollama_generate(opaque, request, result);
}

static int ollama_summarize(
	void * opaque,
	const razion_ai_provider_request_t * request,
	razion_ai_provider_result_t * result) {
	return ollama_generate(opaque, request, result);
...
static int unsupported_result(
	...
	strncpy(result->output, "This Ollama adapter capability is not implemented.",
		sizeof(result->output) - 1);
	return result->status;
}
...
static uint32_t ollama_capabilities(void * opaque) {
	(void)opaque;
...
static void set_response(
	...
	strncpy(response->provider, "ollama", sizeof(response->provider) - 1);
	if (message) {
...
static void handle_request(
	...
	if (request->operation == RAZION_AI_OP_PROVIDER_HEALTH) {
		char detail[RAZION_AI_PROVIDER_MAX_DETAIL];
		if (provider_interface.health(&ollama, detail, sizeof(detail))) {
			...
			response->granted_capabilities =
				provider_interface.capabilities(&ollama);
		}
		return;
	}
	...
	if (request->operation == RAZION_AI_OP_SUMMARIZE) {
		status = provider_interface.summarize(
			&ollama, &provider_request, &result);
	} else {
		status = provider_interface.chat(&ollama, &provider_request, &result);
	}
...
static int run_service(void) {
	if (provider_interface.initialize(&ollama, OLLAMA_CONFIG)) {
		fprintf(stderr, "razion-ai-provider-ollama: unsafe or invalid configuration\n");
		return 1;
	}

	FILE * endpoint = pex_bind(OLLAMA_ENDPOINT);
	if (!endpoint) {
		fprintf(stderr, "razion-ai-provider-ollama: unable to bind %s: %s\n",
			OLLAMA_ENDPOINT, strerror(errno));
		provider_interface.shutdown(&ollama);
		return 1;
	}
...
int main(int argc, char * argv[]) {
	...
	if (getuid() != 0) {
		fprintf(stderr,
			"razion-ai-provider-ollama: only root may run the system service\n");
		return 1;
	}
