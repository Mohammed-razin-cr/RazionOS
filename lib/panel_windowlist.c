	...
	if (window_list) {
		foreach(node, window_list) {
			...
			if (ad->flags & 1) {
				...
				draw_rounded_rectangle(subctx, 2, 3, w - 4, subctx->height - 6, 5, this->pctx->color_widget_bg_active);
				draw_rectangle(subctx, 8, subctx->height - 2, w - 16, 2, this->pctx->color_special);
