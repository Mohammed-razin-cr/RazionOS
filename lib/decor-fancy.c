...
#define BORDER_COLOR   RAZION_BORDER

/* Button and title colors */
#define ACTIVE_COLOR   RAZION_TEXT_PRIMARY
#define INACTIVE_COLOR RAZION_TEXT_SECONDARY
...
static void render_decorations_fancy(yutani_window_t * window, gfx_context_t * ctx, char * title, int decors_active) {
	...
	if ((window->decorator_flags & DECOR_FLAG_TILED)) {
		...
		uint32_t clear_color = RAZION_BACKGROUND;
...
#define EXTRA_SPACE 120
	...
	uint32_t h_color = RAZION_SURFACE_HOVER;
	uint32_t d_color = RAZION_SELECTION;
